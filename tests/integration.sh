...
EOF
    ...
    cat > "$repo/.looper/test/prd.json" <<'JSON'
{
  "schemaVersion": 2,
  "project": "Fixture",
...
test_happy_path() {
    ...
    [ "$(jq -r '.userStories[0].passes' "$repo/.looper/test/prd.json")" = true ] || fail "happy path did not pass story"
...
EOF
    ...
    [ "$(jq -r '.userStories[0].passes' "$repo/.looper/test/prd.json")" = false ] || fail "failed commit marked story passed"
    ...
    [ "$(jq -r '.userStories[0].passes' "$repo/.looper/test/prd.json")" = true ] || fail "commit resume did not pass story"
}
...
test_planning_side_effect_guard() {
    ...
    [ "$(jq -r '.userStories[0].passes' "$repo/.looper/test/prd.json")" = false ] || fail "side-effect planner advanced story"
}
...
test_active_lock_beats_reordered_prd() {
    ...
    tmp="$repo/.looper/test/prd.next.json"
    jq '.userStories[0].priority=99
      | .userStories = [(.userStories[0] | .id="TST-002" | .title="Higher priority replacement candidate" | .priority=1
          | .acceptanceCriteria=[{"id":"TST-002-AC-001","text":"Candidate exists."}]
          | .proofExpectations=[]), .userStories[0]]' "$repo/.looper/test/prd.json" > "$tmp"
    mv "$tmp" "$repo/.looper/test/prd.json"
    run_looper "$repo" "$bin" >/dev/null || true
    [ "$(git -C "$repo" log -1 --pretty=%s)" = "TST-001: Create fixture output" ] || fail "resume switched to reordered live PRD story"
}
...
test_terminal_planning_disposition() {
    ...
    [ "$(jq -r '.userStories[0].execution.status' "$repo/.looper/test/prd.json")" = "$disposition" ] || fail "$disposition PRD execution state was not persisted"
